...
export async function getRecentAssets(months = 5) {
  ...
  const data = await fetchAll<{ snapshot_date: string; asset_type: string; assettype: string | null; owner: string; balance: number }>(
    supabase, 'assets', 'snapshot_date, asset_type, assettype, owner, balance',
    q => q.in('snapshot_date', uniqueDates).order('assettype', { ascending: true })
  )

  const OWNER_NAMES: Record<string, string> = { '운섭': '운섭', '아름': '아름', '공동': '공동소유' }
  const assettypeMap = new Map<string, { asset_type: string; owner: string; balances: Map<string, number> }>()
  for (const a of data) {
    const key = `${a.assettype ?? '기타'}|${a.owner}`
    const cur = assettypeMap.get(key) ?? { asset_type: a.asset_type, owner: a.owner, balances: new Map() }
    cur.balances.set(a.snapshot_date, (cur.balances.get(a.snapshot_date) ?? 0) + a.balance)
    assettypeMap.set(key, cur)
  }

  const assets = Array.from(assettypeMap.entries()).map(([_, { asset_type, owner, balances }]) => ({
    assettype: _.split('|')[0], owner: OWNER_NAMES[owner] || owner, asset_type, balances: uniqueDates.map(d => balances.get(d) ?? 0),
  }))
